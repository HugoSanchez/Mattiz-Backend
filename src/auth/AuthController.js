...
router.post('/register', async (req, res) => {
    ...
    let smartContractWalletAddress = process.env.ROPSTEN_ETH_KEY
...
router.post('/identify', (req, res) => {
    ...
    if (!token) return res.status(200).sendEnc({ auth: false, message: 
        'No token provided.' });
    ...
    jwt.verify(token, process.env.SECRET, (err, decoded) => {
        ...
        User.findById(decoded.id, { password: 0 }, (err, user) => {
            ...
            res.status(200).sendEnc({ auth: true, user: user })
        });
    });
});
